    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if result.get("success"):
                ...
                print(f"   Artist: {result.get('metadata', {}).get('artist_name', 'N/A')}")
                ...
                print(f"   Listing ID: {result.get('draft', {}).get('listing_id', 'N/A')}")
                print(f"   Status: {result.get('draft', {}).get('status', 'N/A')}")
                
                if result.get('draft', {}).get('listing_id'):
                    listing_url = f"https://www.ebay.com/itm/{result['draft']['listing_id']}"
...
async def process_all_upcs():
    ...
    if successful:
        print("\n📋 SUCCESSFUL LISTINGS:")
        for r in successful:
            print(f"   • {r['upc']}: {r.get('artist', 'N/A')} - {r.get('album', 'N/A')}")
            if r.get('listing_id'):
    ...
    results_file = f"sample_upcs_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"

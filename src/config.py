...
class Settings(BaseSettings):
    ...
    gcp_project_id: str = Field(
        default=os.getenv("GCP_PROJECT_ID", "draft-maker-468923"), env="GCP_PROJECT_ID"
    )
    gcp_region: str = Field(
        default=os.getenv("GCP_REGION", "us-west1"), env="GCP_REGION"
    )
    storage_bucket_name: str = Field(
        default=os.getenv("STORAGE_BUCKET_NAME", "draft-maker-bucket"),
    ...
    firestore_collection_tokens: str = Field(
        default="api_tokens", env="FIRESTORE_COLLECTION_TOKENS"
    )
    firestore_collection_processed_files: str = Field(
        default="processed_files", env="FIRESTORE_COLLECTION_PROCESSED_FILES"
    )
    firestore_collection_draft_listings: str = Field(
        default="draft_listings", env="FIRESTORE_COLLECTION_DRAFT_LISTINGS"
    )
    ...
    ebay_client_secret: str = Field(
        default=os.getenv("EBAY_CLIENT_SECRET", ""), env="EBAY_CLIENT_SECRET"
    )

    # Spotify
    spotify_client_id: str = Field(
        default=os.getenv("SPOTIFY_CLIENT_ID", ""), env="SPOTIFY_CLIENT_ID"
    )
    ...
    environment: str = Field(
        default=os.getenv("ENVIRONMENT", "development"), env="ENVIRONMENT"
    )
    ...
    ebay_fulfillment_policy_id: str = Field(
        default="381603015022", env="EBAY_FULFILLMENT_POLICY_ID"
    )
    ebay_payment_policy_id: str = Field(
        default="345889112022", env="EBAY_PAYMENT_POLICY_ID"
    )
    ebay_return_policy_id: str = Field(
        default="345889054022", env="EBAY_RETURN_POLICY_ID"
    )
    ebay_category_id: str = Field(
        default="176984", env="EBAY_CATEGORY_ID"
    )  # Music CDs category
    ...
    if settings.environment.lower() == "production":
        try:
            ...
            if secrets.get("discogs_personal_access_token"):
                settings.discogs_personal_access_token = secrets[

...
class ListingOrchestrator:
    ...
    async def process_single_upc(
        ...
        try:
            ...
            logger.info(f"[{upc}] Metadata found: {metadata.get('artist_name')} - {metadata.get('title')}")
            ...
            if not pricing or not pricing.get("recommended_price"):
                ...
                logger.info(f"[{upc}] Pricing found: ${pricing['recommended_price']:.2f} (confidence: {pricing.get('confidence', 'unknown')})")
...
async def main():
    ...
    parser.add_argument(
        "--local",
        action="store_true",
        help="Use local file instead of GCS"
    )
    parser.add_argument(
        "--test",
        action="store_true",
    ...
    parser.add_argument(
        "--single",
        type=str,
        help="Process a single UPC for testing"
    )

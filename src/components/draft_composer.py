...
class DraftComposer:
    ...
    def __init__(self):
        ...
        template_path = (
            Path(__file__).parent.parent.parent / "data" / "listing_payload.json"
        )
        with open(template_path, "r") as f:
            self.listing_template = json.load(f)

    async def create_draft_listing(
        self, metadata: Dict[str, Any], images: Dict[str, Any], pricing: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            inventory_success = await self._create_inventory_item(
                sku, inventory_payload, access_token
            )
            ...
            if offer_result and "offerId" in offer_result:
                ...
                logger.info(
                    f"Publishing offer {offer_result['offerId']} to create live listing..."
                )
                listing_result = await self._publish_offer(
                    offer_result["offerId"], access_token
                )
                ...
                if listing_result and "listingId" in listing_result:
                    ...
                    logger.info(
                        f"Successfully published listing {listing_result['listingId']} for UPC {upc}"
                    )
                else:
                    logger.warning(
                        f"Offer created but not published for UPC {upc}. It remains as an unpublished offer."
                    )
                    result["status"] = "unpublished"
            else:
    ...
    def _build_inventory_item(
        ...
        logger.debug(
            f"Building title with metadata - Artist: {artist}, Album: {album}, Year: {year}"
        )
    ...
    def _build_offer(self, sku: str, pricing: Dict[str, Any]) -> Dict[str, Any]:
        ...
        logger.info(
            f"Best Offer enabled: {offer['pricingSummary'].get('bestOfferEnabled', False)}"
        )
        ...
        offer["listingPolicies"][
            "fulfillmentPolicyId"
        ] = "381603015022"  # CD Combined Shipping
    ...
    def _build_description(
        self, metadata: Dict[str, Any], pricing: Dict[str, Any]
    ) -> str:
        """
        ...
        description_parts = [f"<h3>{artist} - {album}</h3>"]
        ...
        if metadata.get("tracks") and len(metadata["tracks"]) > 0:
            ...
            if len(metadata["tracks"]) > 20:
                description_parts.append(
                    f"<li>... and {len(metadata['tracks']) - 20} more tracks</li>"
                )
            description_parts.append("</ol>")
        ...
        if catalog:
            description_parts.append(
                f"<p><strong>Catalog Number:</strong> {catalog}</p>"
            )
    ...
    async def _create_inventory_item(
        self, sku: str, payload: Dict[str, Any], access_token: str
    ) -> bool:
        """
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code in [200, 201, 204]:
                    logger.info(
                        f"Successfully created/updated inventory item with SKU: {sku}"
                    )
                    return True
                else:
                    logger.error(
                        f"Failed to create inventory item. Status: {response.status_code}"
                    )
    ...
    async def _create_offer(
        ...
        logger.info(
            f"  PricingSummary: {json.dumps(payload.get('pricingSummary', {}), indent=2)}"
        )
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code in [200, 201]:
                    ...
                    logger.info(
                        f"Successfully created offer with ID: {result.get('offerId')}"
                    )

                    # Log what eBay returned
                    if "pricingSummary" in result:
                        logger.info(
                            f"Returned pricingSummary: {json.dumps(result['pricingSummary'], indent=2)}"
                        )

                    return result
                else:
                    logger.error(
                        f"Failed to create offer. Status: {response.status_code}"
                    )
                    logger.error(f"Response: {response.text}")
                    return None
    ...
    async def _publish_offer(
        self, offer_id: str, access_token: str
    ) -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    result = response.json()
                    logger.info(
                        f"Successfully published offer as listing ID: {result.get('listingId')}"
                    )
                    return result
                else:
                    logger.error(
                        f"Failed to publish offer. Status: {response.status_code}"
                    )
                    logger.error(f"Response: {response.text}")
                    return None

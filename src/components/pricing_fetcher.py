...
class PricingFetcher:
    ...
    async def fetch_pricing(self, upc: str, metadata: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        if metadata:
            ...
            if metadata.get("title"):
                search_strategies.append({
                    "method": "album_only",
                    "keywords": f'{metadata["title"]} CD'
                })
        
        # Try each search strategy
        for strategy in search_strategies:
            logger.info(f"Trying search strategy: {strategy['method']} with keywords: {strategy['keywords']}")
            ...
            if sold_items:
                ...
                pricing_result = self._calculate_pricing_stats(sold_items, pricing_result)
        ...
        avg_price_str = f"{pricing_result['average_price']:.2f}" if pricing_result['average_price'] else "0.00"
        logger.info(
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                if finding_response.get("ack", ["Failure"])[0] != "Success":
                    error_msg = finding_response.get("errorMessage", [{}])[0].get("error", [{}])[0].get("message", ["Unknown error"])[0]
                ...
                logger.info(f"Found {len(sold_items)} sold items for keywords: {keywords}")
                return sold_items
    ...
    def _calculate_pricing_stats(self, sold_items: List[Dict[str, Any]], pricing_result: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        if len(prices) >= 4:
            ...
            if len(filtered_prices) >= 3:
                ...
                logger.debug(f"Filtered outliers: kept {len(prices)} of {len(sold_items)} prices")
    ...
    def _calculate_recommended_price(self, pricing_result: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        if pricing_result["sample_size"] == 0:
            ...
            pricing_result["recommendation_reason"] = "No sold data found, using default price"
            
        elif pricing_result["confidence"] == "high":
            # High confidence - use slightly below average for competitive pricing
            pricing_result["recommended_price"] = round(pricing_result["average_price"] * 0.95, 2)
            pricing_result["price_strategy"] = "competitive"
            pricing_result["recommendation_reason"] = f"Based on {pricing_result['sample_size']} recent sales, priced competitively at 95% of average"
            
        elif pricing_result["confidence"] == "medium":
            # Medium confidence - use median as it's more robust
            pricing_result["recommended_price"] = round(pricing_result["median_price"], 2)
            pricing_result["price_strategy"] = "median"
            pricing_result["recommendation_reason"] = f"Based on {pricing_result['sample_size']} recent sales, using median price"
            
        else:
            # Low confidence - be conservative
            if pricing_result["median_price"]:
                pricing_result["recommended_price"] = round(pricing_result["median_price"] * 0.9, 2)
                pricing_result["price_strategy"] = "conservative"
                pricing_result["recommendation_reason"] = f"Limited data ({pricing_result['sample_size']} sales), using conservative pricing"
            else:
                pricing_result["recommended_price"] = 9.99
                pricing_result["price_strategy"] = "default"
                pricing_result["recommendation_reason"] = "Insufficient data, using default price"
        
        # Ensure price is reasonable
        if pricing_result["recommended_price"]:
            # Minimum $3.99 (to cover shipping costs)
            pricing_result["recommended_price"] = max(3.99, pricing_result["recommended_price"])
            # Maximum $49.99 (reasonable for most CDs)
            pricing_result["recommended_price"] = min(49.99, pricing_result["recommended_price"])

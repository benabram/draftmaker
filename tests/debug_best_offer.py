        ...
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 200:
                ...
                if pricing.get("bestOfferAutoAcceptPrice"):
                    print(
                        f"  Auto Accept Price: ${pricing['bestOfferAutoAcceptPrice'].get('value')}"
                    )
                if pricing.get("bestOfferAutoDeclinePrice"):
                    print(
                        f"  Auto Decline Price: ${pricing['bestOfferAutoDeclinePrice'].get('value')}"
                    )
...
async def test_create_offer_with_best_offer(access_token: str):
    ...
    inventory_payload = {
        "product": {
            ...
            "aspects": {"Format": ["CD"], "Artist": ["Test Artist"], "Genre": ["Rock"]},
    ...
    async with httpx.AsyncClient() as client:
        ...
        offer_payload = {
            ...
            "bestOfferTerms": {  # CORRECT: Best Offer at offer level, not in pricingSummary
                ...
                "autoAcceptPrice": {"value": "15.00", "currency": "USD"},
        ...
        print(
            f"Pricing Summary: {json.dumps(offer_payload['pricingSummary'], indent=2)}"
        )
        print(
            f"Best Offer Terms: {json.dumps(offer_payload.get('bestOfferTerms', {}), indent=2)}"
        )

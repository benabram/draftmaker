...
test_pricing = {"recommended_price": 12.99}

test_images = {"primary_image": "https://example.com/image.jpg"}
...
def test_offer_creation():
    ...
    print(
        "2. Shallow copy bestOfferEnabled after modification:",
    ...
    print(
        "3. Deep copy bestOfferEnabled after modification:",
...
def check_metadata_source():
    ...
    print(
        f"  year: {test_metadata.get('year')} (type: {type(test_metadata.get('year'))})"
    )
    print(
        f"  release_date: {test_metadata.get('release_date')} (type: {type(test_metadata.get('release_date'))})"
    )
    print(
        f"  released: {test_metadata.get('released')} (type: {type(test_metadata.get('released'))})"
    )

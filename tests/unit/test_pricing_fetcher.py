...
async def test_pricing_fetch(upc: str):
    ...
    try:
        ...
        if pricing_result["sample_size"] > 0:
            ...
            print(
                f"  Price Range: ${pricing_result['min_price']:.2f} - ${pricing_result['max_price']:.2f}"
            )
            ...
            if pricing_result.get("sample_listings"):
                ...
                for i, listing in enumerate(pricing_result["sample_listings"][:3], 1):
                    ...
                    print(
                        f"     Price: ${listing['price']:.2f} | Condition: {listing['condition']}"
                    )
        else:
            print("\n❌ No sold data found")
        ...
        return (
            pricing_result["sample_size"] > 0
            or pricing_result["recommended_price"] is not None
        )

...
class TestIntegration:
    ...
    async def test_complete_pipeline_success(
        self, sample_metadata, sample_pricing, sample_images, sample_draft_result
    ):
        ...
        with patch("src.orchestrator.get_upc_processor") as mock_upc_getter:
            with patch("src.orchestrator.get_metadata_fetcher") as mock_meta_getter:
                with patch("src.orchestrator.get_pricing_fetcher") as mock_price_getter:
                    with patch(
                        "src.orchestrator.get_image_fetcher"
                    ) as mock_image_getter:
                        with patch(
                            "src.orchestrator.get_draft_composer"
                        ) as mock_draft_getter:
                            ...
                            mock_draft.create_draft_listing.return_value = (
                                sample_draft_result
                            )
                            ...
                            with patch.object(
                                orchestrator, "_save_results", new_callable=AsyncMock
                            ):
                                # Process batch
    ...
    async def test_pipeline_with_failures(self):
                ...
                with patch("src.orchestrator.get_pricing_fetcher") as mock_price_getter:
                    with patch(
                        ...
                        with patch(
                            ...
                            mock_price = AsyncMock()
                            mock_price.fetch_pricing.return_value = {
        ...
        assert (
            result["failed"] == 2
        )  # Second had no metadata, third failed draft creation

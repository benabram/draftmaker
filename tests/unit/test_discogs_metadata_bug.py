...
class DiscogsMetadataBugTester:
    ...
    async def test_metadata_fetching_flow(self, upc: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if musicbrainz_data:
                logger.info(
                    f"✓ MusicBrainz returned data: {json.dumps(musicbrainz_data, indent=2)[:500]}..."
                )
                ...
                result["musicbrainz_response"] = {"found": False, "data": None}
            ...
            if discogs_data:
                logger.info(
                    f"✓ Discogs returned data: {json.dumps(discogs_data, indent=2)[:500]}..."
                )
            ...
            if full_metadata and full_metadata.get("is_complete"):
                logger.info(
                    f"✓ Full metadata fetch successful: {full_metadata.get('artist_name')} - {full_metadata.get('title')}"
                )
            else:
            ...
            orchestrator_result = await self.orchestrator.process_single_upc(
                upc, create_draft=False
            )
            ...
            if orchestrator_result.get("success"):
                ...
                logger.warning(
                    f"✗ Orchestrator processing failed: {orchestrator_result.get('error')}"
                )
                result["errors"].append(
                    f"Orchestrator failed: {orchestrator_result.get('error')}"
                )
            ...
            if full_metadata and full_metadata.get("is_complete"):
                ...
                draft_result = await self.draft_composer.create_draft_listing(
                    metadata=full_metadata, images=images, pricing=pricing
                )
                ...
                if draft_result.get("success"):
                    logger.info(
                        f"✓ Draft creation successful - SKU: {draft_result.get('sku')}"
                    )
                    if draft_result.get("listing_id"):
                        logger.info(
                            f"✓ Listing published - ID: {draft_result.get('listing_id')}"
                        )
                    else:
    ...
    async def run_full_test(self):
        ...
        for result in results:
            upc = result["upc"]
            mb_found = (
                result["musicbrainz_response"]["found"]
                if result["musicbrainz_response"]
                else False
            )
            discogs_found = (
                result["discogs_response"]["found"]
                if result["discogs_response"]
                else False
            )
            combined_complete = (
                result["combined_metadata"]["is_complete"]
                if result["combined_metadata"]
                else False
            )
            ...
            if result.get("draft_creation"):
                ...
                if draft.get("success"):
                    ...
                    if draft.get("listing_id"):
                        logger.info(
                            f"  Listing: Published (ID: {draft.get('listing_id')})"
                        )
                else:

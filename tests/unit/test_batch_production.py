...
def check_job_status(job_id):
    ...
    """
    url = (
        f"https://draft-app-467925563302.us-central1.run.app/api/batch/status/{job_id}"
    )
    ...
    for i in range(max_checks):
        ...
        if job_status == "completed":
            ...
            if status.get("total_upcs"):
                print(
                    f"  Progress: {status.get('successful', 0) + status.get('failed', 0)}/{status.get('total_upcs')} UPCs processed"
                )
        ...
        if response.status_code == 200:
            ...
            for job in jobs:
                ...
                if job.get("total_upcs"):
                    print(
                        f"  Results: {job.get('successful', 0)}/{job.get('total_upcs')} successful"
                    )
                if job.get("error"):

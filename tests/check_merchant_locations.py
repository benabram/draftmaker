        ...
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 200:
                ...
                if locations:
                    ...
                    print(
                        "   2. Or create one via API using POST /sell/inventory/v1/location/{merchantLocationKey}"
                    )
                    print()
                    ...
                    create_response = await client.post(
                        create_url, headers=headers, json=location_data, timeout=30.0
                    )

                    if create_response.status_code in [200, 201, 204]:
                        print(
                            f"   ✅ Successfully created default merchant location: {location_key}"
                        )
                    else:
                        print(
                            f"   ❌ Failed to create location: {create_response.status_code}"
                        )

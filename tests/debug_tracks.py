...
async def test_musicbrainz_search(upc):
    ...
    async with httpx.AsyncClient() as client:
        ...
        response = await client.get(search_url, params=search_params, headers=headers, timeout=30.0)
            ...
            if data.get("releases") and len(data["releases"]) > 0:
                ...
                if release.get("media"):
                    for i, medium in enumerate(release["media"]):
                        ...
                        print(f"   Medium {i+1}: {len(tracks)} tracks returned, {track_count} total tracks")
                        if tracks:
                ...
                if mbid and not release.get("media", [{}])[0].get("tracks"):
                    ...
                    response2 = await client.get(release_url, params=release_params, headers=headers, timeout=30.0)
                        ...
                        if full_release.get("media"):
                            ...
                            for i, medium in enumerate(full_release["media"]):
                                ...
                                if tracks and len(tracks) > 0:
                                    for j, track in enumerate(tracks[:3]):  # Show first 3
                                        print(f"     Track {j+1}: {track.get('title', 'N/A')}")
                                    if len(tracks) > 3:
                            ...
                            if total_tracks == 0:
                                ...
                                print("   This means we need to fetch recordings separately!")
            else:

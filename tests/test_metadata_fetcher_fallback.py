...
async def test_musicbrainz_empty_response():
    ...
    for upc in test_upcs:
        ...
        print(
            f"   MusicBrainz: Title={mb_data.get('title')}, Artist={mb_data.get('artist_name')}"
        )
        print(
            f"   Discogs:     Title={dc_data.get('title')}, Artist={dc_data.get('artist_name')}"
        )
        ...
        if (
            not mb_has_title
            and not mb_has_artist
            and "musicbrainz" in combined.get("metadata_sources", [])
        ):
            print(
                "   ❌ BUG FOUND: MusicBrainz listed in sources despite returning no useful data"
            )
        else:
        ...
        if (
            dc_has_title
            and not mb_has_title
            and combined.get("title") != dc_data.get("title")
        ):
            print(
                "   ❌ BUG FOUND: Discogs title not used when MusicBrainz has no title"
            )
        else:
        ...
        if (
            dc_has_artist
            and not mb_has_artist
            and combined.get("artist_name") != dc_data.get("artist_name")
        ):
            print(
                "   ❌ BUG FOUND: Discogs artist not used when MusicBrainz has no artist"
            )
        else:
...
async def test_full_pipeline():
    ...
    # Analyze the results
    print("\nAnalysis:")
    if metadata.get("mbid") is None and "musicbrainz" in metadata.get(
        "metadata_sources", []
    ):

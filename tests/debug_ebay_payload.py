...
def check_offer_payload():
    ...
    if "pricingSummary" in offer:
        if "bestOfferEnabled" in offer["pricingSummary"]:
            print(f"   ✓ bestOfferEnabled is present: {offer['pricingSummary']['bestOfferEnabled']}")
        else:
    ...
    print(f"   bestOfferEnabled still present: {offer['pricingSummary'].get('bestOfferEnabled', 'MISSING!')}")

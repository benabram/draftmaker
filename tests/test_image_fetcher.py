...
async def test_image_fetch(upc: str):
    ...
    try:
        ...
        print(f"  Primary Image: {'✅' if images_result.get('primary_image') else '❌'}")
        ...
        for i, img in enumerate(images_result['images'][:3], 1):  # Show first 3
            ...
            if source == 'cover_art_archive':
                ...
                print(f"     - Has 500px thumbnail: {'✅' if img.get('thumbnail_500') else '❌'}")
...
async def main():
    ...
    test_cases = [
        settings.test_upc_codes[0],  # First test UPC (likely no MBID based on previous test)
    ...
    print("Note: Testing both scenarios - with MBID (Cover Art) and without (Spotify only)")

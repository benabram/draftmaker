...
async def test_best_offer_payload():
    ...
    inventory_payload = composer._build_inventory_item(
        test_metadata, test_images, test_pricing
    )
    ...
    if "bestOfferEnabled" in offer_payload:
        print(
            f"   ⚠️ bestOfferEnabled at offer level: {offer_payload['bestOfferEnabled']}"
        )
    else:
    ...
    async with httpx.AsyncClient() as client:
        ...
        print(
            f"   Sending pricingSummary: {json.dumps(offer_payload['pricingSummary'], indent=2)}"
        )
        ...
        if response.status_code in [200, 201]:
            ...
            if response.status_code == 200:
                ...
                print(
                    f"   eBay returned pricingSummary: {json.dumps(pricing_summary, indent=2)}"
                )

                if "bestOfferEnabled" in pricing_summary:
                    print(
                        f"   ✓ Best Offer Enabled: {pricing_summary['bestOfferEnabled']}"
                    )
                else:
